...
class Edge:
    ...
    """
    def __init__(self, id: str, left_bound: list, right_bound: list, lane_divider: List[list]):
        self.id = id
...
class NormalLine:
    ...
    def __init__(self, lane: ET.Element, lane_width: float = 3.2):
        ...
        # self.left_bound = self.get_bound_lane(-self.lane_width/2)
        # self.right_bound = self.get_bound_lane(self.lane_width/2)
...
class XML2JSON:
    ...
    def draw_drivable_area(self, position_list: List[list]):
        ...
        nusc_map["drivable_area"].append({
            "token": str(uuid.uuid4()),
            "polygon_tokens": [polygon_token]
        })
    ...
    def draw_junction(self, junction: ET.Element):
        ...
        if drivable:
            ...
            self.draw_drivable_area(position_list)
        else:
            # 将其绘制为walkway
            shape = junction.attrib["shape"].split(" ")
            position_list = []
    ...
    def draw_edge(self):
        ...
        for edge_id, edge in self.edges.items():
            if edge_id in incorporation_id:
                continue
            if edge_id[0] == "-":
                opposite_edge_id = edge_id[1:]
            else:
                opposite_edge_id = "-"+edge_id
            if opposite_edge_id in self.edges.keys():
                # 合并两个edge
                opposite_edge = self.edges[opposite_edge_id]
                # 计算left和right的距离，选择小的那个作为重合的边
                left_distance = np.linalg.norm(np.array(edge.left_bound[0])-np.array(opposite_edge.left_bound[-1]))
                right_distance = np.linalg.norm(np.array(edge.right_bound[0])-np.array(opposite_edge.right_bound[-1]))

                if left_distance < right_distance and left_distance < 1.0:
                    self.edges[edge_id].road_divider = edge.left_bound[::]
                    self.edges[edge_id].left_bound = opposite_edge.right_bound[::-1]
                    self.edges[edge_id].lane_divider.extend(opposite_edge.lane_divider[::])
                    incorporation_id.append(opposite_edge_id)
                elif right_distance < 1.0:
                    self.edges[edge_id].road_divider = edge.right_bound[::]
                    self.edges[edge_id].right_bound = opposite_edge.left_bound[::-1]
                    self.edges[edge_id].lane_divider.extend(opposite_edge.lane_divider[::])
                    incorporation_id.append(opposite_edge_id)
                else:
                    continue
